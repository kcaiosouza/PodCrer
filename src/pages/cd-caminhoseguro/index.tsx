...
export default function cdcaminhoseguro({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Caminho Seguro | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdcaminhoseguro', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

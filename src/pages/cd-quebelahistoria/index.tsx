...
export default function cdquebelahistoria({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Que Bella Historia | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdquebelahistoria', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

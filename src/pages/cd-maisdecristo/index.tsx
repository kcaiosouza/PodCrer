...
export default function cdmaisdecristo({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Mais de Cristo | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdmaisdecristo', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

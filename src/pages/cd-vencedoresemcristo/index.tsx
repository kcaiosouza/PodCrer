...
export default function cdvencedoresemcristo({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Vencedores em Cristo | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdvencedoresemcristo', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

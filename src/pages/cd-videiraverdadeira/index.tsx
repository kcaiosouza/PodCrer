...
export default function cdvideiraverdadeira({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Videira Verdadeira | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdvideiraverdadeira', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

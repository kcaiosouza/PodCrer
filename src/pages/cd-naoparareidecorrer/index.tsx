...
export default function cdnaoparareidecorrer({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Não Pararei de Correr | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdnaoparareidecorrer', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

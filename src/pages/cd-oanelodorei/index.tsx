...
export default function cdoanelodorei({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD O Anelo do Rei | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdoanelodorei', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

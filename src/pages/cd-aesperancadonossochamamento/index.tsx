...
export default function cdaesperancadonossochamamento({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD A Esperança do Nosso Chamamento | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdaesperancadonossochamamento', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

...
export default function cdesplendidavidadaigreja({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Esplêndida Vida da Igreja | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdesplendidavidadaigreja', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

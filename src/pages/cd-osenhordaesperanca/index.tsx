...
export default function cdosenhordaesperanca({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD O Senhor da Esperança | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdosenhordaesperanca', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

...
export default function cdapenasteolhar({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Apenas Te Olhar | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdapenasteolhar', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

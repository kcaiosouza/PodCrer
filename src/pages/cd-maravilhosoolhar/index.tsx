...
export default function cdmaravilhosoolhar({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Maravilhoso Olhar | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdmaravilhosoolhar', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

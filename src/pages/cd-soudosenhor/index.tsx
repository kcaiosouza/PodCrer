...
export default function cdsoudosenhor({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Sou do Senhor | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdsoudosenhor', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

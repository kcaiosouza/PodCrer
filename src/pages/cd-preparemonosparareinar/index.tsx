...
export default function cdpreparemonosparareinar({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Preparémonos para Reinar | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdpreparemonosparareinar', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

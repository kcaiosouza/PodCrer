...
export default function cdalpercibirlasaguas({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title> CD Alpercibir Las Aguas | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdalpercibirlasaguas', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

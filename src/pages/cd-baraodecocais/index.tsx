...
export default function cdyahayvictoria({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Igreja em Barão de Cocais | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdbaraodecocais', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

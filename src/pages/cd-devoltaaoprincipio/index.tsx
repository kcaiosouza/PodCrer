...
export default function cddevoltaaoprincipio({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title> CD De Volta ao Princípio | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cddevoltaaoprincipio', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

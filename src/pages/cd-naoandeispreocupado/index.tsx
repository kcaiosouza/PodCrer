...
export default function cdnaoandeispreocupado({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Não Andeis Preocupados | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdnaoandeispreocupado', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

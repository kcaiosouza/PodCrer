...
export default function cdvengojesusati({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Vengo Jesus a Ti | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdvengojesusati', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

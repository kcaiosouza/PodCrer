...
export default function cdseacercaeldia({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Se Acerca El Día | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdseacercaeldia', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

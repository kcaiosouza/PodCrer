...
export default function cdjovenssantos({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Jovens Santos | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdjovenssantos', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

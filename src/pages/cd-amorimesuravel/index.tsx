...
export default function cdamorimesuravel({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Amor Imesurável | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdamorimesuravel', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

...
export default function cddeusabencoeaafrica({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Deus Abençoe a África | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cddeusabencoeaafrica', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

...
export default function cdsalvopeloinvocar({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Salvos pelo Invocar | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdsalvopeloinvocar', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

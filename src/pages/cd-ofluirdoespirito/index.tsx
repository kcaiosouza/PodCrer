...
export default function cdofluirdoespirito({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD O Fluir do Espírito | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdofluirdoespirito', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

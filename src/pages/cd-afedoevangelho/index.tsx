...
export default function cdafedoevangelho({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD A Fé do Evangelho | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdafedoevangelho', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

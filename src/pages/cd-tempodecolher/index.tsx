...
export default function cdtempodecolher({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Tempo de Colher | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdtempodecolher', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

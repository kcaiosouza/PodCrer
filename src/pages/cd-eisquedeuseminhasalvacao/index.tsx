...
export default function cdeisquedeuseminhasalvacao({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Eis que Deus é minha Salvação | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdeisquedeuseminhasalvacao', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

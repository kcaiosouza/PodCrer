...
export default function cdtuvivapresencia({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Tu Viva Presencia | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdtuvivapresencia', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

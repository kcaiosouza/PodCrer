...
export default function cdnatualuzvemosaluz({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Na Tua Luz Vemos a Luz | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdnatualuzvemosaluz', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

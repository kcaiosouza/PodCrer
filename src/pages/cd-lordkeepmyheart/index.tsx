...
export default function cdlordkeepmyheart({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Lord Keep My Heart | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdlordkeepmyheart', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

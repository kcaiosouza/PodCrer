...
export default function cdbemaventurado({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Bem Aventurado os limpos de Coração | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdbemaventurado', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

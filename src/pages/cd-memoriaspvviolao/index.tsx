...
export default function cdmemoriaspvviolao({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Memórias (Piano - Violão - Voz) | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdmemoriaspvviolao', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

...
export default function cdgozoinefable({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Gozo Inefable | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdgozoinefable', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

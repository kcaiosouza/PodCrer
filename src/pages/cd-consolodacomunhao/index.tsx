...
export default function cdconsolodacomunhao({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Consolo da Comunhão | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdconsolodacomunhao', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

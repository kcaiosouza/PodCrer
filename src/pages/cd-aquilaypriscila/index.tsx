...
export default function cdaquilaypriscila({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Aquila y Priscila | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdaquilaypriscila', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

...
export default function cdnombresobretodonombre({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Nombre Sobre Todo Nombre | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdnombresobretodonombre', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

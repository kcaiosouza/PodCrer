...
export default function cdcristonossaesperanca({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Cristo nossa Esperança | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdcristonossaesperanca', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

...
export default function cdserumjovemparadeus({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>Cd Ser um jovem para Deus | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdserumjovemparadeus', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

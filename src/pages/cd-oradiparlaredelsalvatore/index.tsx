...
export default function cdoradiparlaredelsalvatore({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Ora di Parlare del Salvatore | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdoradiparlaredelsalvatore', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',

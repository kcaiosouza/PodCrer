...
export default function cdaneloportuapresenca({ latestEpisodes, allEpisodes }: HomeProps) {
  const emojiChoose = [
    '🎵',
    '🎶',
    '📻',
  ...
  return (
    <HomepageComponent>
      <ToastContainer />
      <Head>
        <title>CD Anelo por Tua Presença | IGCGMusic</title>
      </Head>
...
export const getStaticProps: GetStaticProps = async () => {
  const { data } = await api.get('cdaneloportuapresenca', {
    params: {
      _limit: 1000,
      _sort: 'published_at',
      _order: 'desc',
